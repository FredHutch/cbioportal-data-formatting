...
# Make into a MAF
```{r}
annot <- selectedData %>% select(molecular_id, seq_flowcell_id, age_range, tissue_type, seq_pool, study_id, subject_id, biospecimen_id, assay_material_id) %>% mutate(Tumor_Sample_Barcode = paste(molecular_id, assay_material_id, sep = "-"), SAMPLE_ID = paste(molecular_id, biospecimen_id, sep = "-"))
...
write_delim(study, file = "./Pag-Validation-Data/uploadPrep/TGRmeta-study.txt", delim = "\t")

unique(metadata$subject_id)
clinical_patients <- metadata %>% group_by(subject_id) %>% summarize(diagnosis, subtype, sex, race ) %>% unique()
write_delim(clinical_patients, file = "./Pag-Validation-Data/uploadPrep/TGRmeta-clinical_patients.txt", delim = "\t")

unique(metadata$biospecimen_id)
clinical_samples <- metadata %>% group_by(biospecimen_id) %>% summarize(collection_event, collection_type, tissue_type_origin, cell_line_id) %>% unique()
write_delim(clinical_samples, file = "./Pag-Validation-Data/uploadPrep/TGRmeta-clinical_samples.txt", delim = "\t")
...
write_delim(genomics_samples, file = "./Pag-Validation-Data/uploadPrep/TGRmeta-genomics_samples.txt", delim = "\t")
